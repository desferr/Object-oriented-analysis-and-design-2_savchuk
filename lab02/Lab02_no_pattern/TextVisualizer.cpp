...
void TextVisualizer::parseText(const QString& text, const QColor& defaultColor)
{
	...
	for (const QChar& ch : text)
	{
		...
		m_matrix[row][column] = std::make_shared<Symbol>(ch, row, column, color);
		column++;
	}
}
...
void TextVisualizer::removeSymbol(int row, int column, const QColor& color)
{
	if (row >= 0 && row < m_rows && column >= 0 && column <= m_columns)
	{
		...
		else m_matrix[row][column] = std::make_shared<Symbol>(' ', row, column, color);
	}
}
...
void TextVisualizer::changeLeftNullToSpace(int row, int column, const QColor& color)
{
	...
	while (column >= 0)
	{
		if (m_matrix[row][column] == nullptr) m_matrix[row][column] = std::make_shared<Symbol>(' ', row, column, color);
		column--;
	}
	return;
}
