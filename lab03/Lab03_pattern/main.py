...
class CardsIterator(Iterator):
    ...
    def __init__(self, aggregate: 'Deck'):
    ...
    def is_done(self) -> bool:
        return self._current_pos >= len(self._order) - 1

    def is_done_reversed(self) -> bool:
        return self._current_pos <= 0

    def current_item(self) -> Item | None:
        if 0 <= self._current_pos < len(self._order):
            return self._aggregate.cards[self._order[self._current_pos]]
        return None

    def prev(self):
        if not self.is_done_reversed():
            self._current_pos -= 1
...
class MyApp(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.setupUi(self)
        self.deck = Deck()
        self.iterator = self.deck.create_iterator()
    ...
    def init_cards(self):
        self.cardLabel.setText(f"{self.iterator.current_item().question}")
        self.cardCountLabel.setText(f"{self.iterator.get_current_index() + 1} / {self.deck.length()}")
    ...
    def prev_card(self):
        if not self.iterator.is_done_reversed():
    ...
    def flip_card(self):
        if self.is_card_flipped:
            ...
            self.cardLabel.setText(f"{self.iterator.current_item().answer}")
            self.is_card_flipped = True


    def next_card(self):
        if not self.iterator.is_done():
            if self.is_card_flipped:
                self.is_card_flipped = False
            self.iterator.next()
            self._update_display()
    ...
    def to_first(self):
        if self.is_animating:
            return
        if self.iterator.is_done_reversed():
            return
        if self.is_card_flipped:
            self.is_card_flipped = False
        self.set_buttons_enabled(False)
    ...
    def edit_card(self):
        if self.is_card_flipped:
            text, ok = QInputDialog.getText(self, "Окно ввода", f"Введите ответ для карточки:\n{self.iterator.current_item().question} : {self.iterator.current_item().answer}")
            if ok and text:
                self.saved_pos = self.iterator.get_current_deck_index()
                self.deck.set_card(self.saved_pos, text, True)
                self.iterator = self.deck.create_iterator()
                self.iterator.set_pos(self.saved_pos)
                self._update_display()
        else:
    ...
    def add_card(self):
        ...
        if ok and text:
            if self.deck.length() == 0:
                ...
                self.saved_pos = self.iterator.get_current_deck_index() + 1
    ...
    def delete_card(self):
        if self.deck.length() != 0:
            reply = QMessageBox.question(self, "Подтверждение", f"Вы уверены, что хотите удалить карточку:\n{self.iterator.current_item().question} : {self.iterator.current_item().answer}", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
    ...
    def save_cards(self):
        ...
        try:
            with open(file_path, 'w', encoding='utf-8') as file:
                self.saved_pos = self.iterator.get_current_index()
                self.iterator.first()
                while True:
                    file.write(f"\"{self.iterator.current_item().question}\" : \"{self.iterator.current_item().answer}\"\n")
                    if self.iterator.is_done():
                        break
                    self.iterator.next()
                self.iterator.set_pos(self.saved_pos)
